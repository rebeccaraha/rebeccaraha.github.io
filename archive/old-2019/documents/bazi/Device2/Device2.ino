...
void setup() {
  ...
  Particle.subscribe("raha2/player1btns", player1btns);
  Particle.subscribe("raha2/player1status", player1status);
  attachInterrupt(D6, magnetDetect, RISING);
  //Rising means sensor is moving away from the magnet
...
//Records button presses as they happen
void letsPlay() {
  if (changingMode == 0) {
    vibrate();
  }
...
void player1btns(const char *event,const char *data) {
  ...
  for(int i = 0; i < bufferOPLen; i++){ //Reads buffer
    ...
    int btn = bufferOP[i];
    Serial.print("flash color from player1btns");
    Serial.println();
    slowerFlash(btn);
...
void player1status (const char *event,const char *data){
  Serial.print("void player1status()");
  Serial.println();
  String newPlayer1StatusEvent= String(event);
  String newPlayer1StatusData = String(data);

  if (newPlayer1StatusData == "lost") {
    deviceStatus = 7;
  }

  else if (newPlayer1StatusData == "play") {
    firstTime = 1;
    deviceStatus = 8;
  }

  Serial.print("player status is ");
  Serial.println(newPlayer1StatusData);
}
     ...
     if (val == bufferOP[pressIndex]) { //Makes sure a number is being added - not a 0
      ...
      //Game Continues
       if (pressIndex == bufferOPLen) { //this has to happen
         clearBuffer();  //used to be clear buffer
...
void getBuffer(){
  ...
  Serial.print(player2btns);
  Serial.println();
  Particle.publish("raha/player2btns", player2btns);
  clearBuffer();
  str = "";
  deviceStatus = 4;
}
...
void publishStatus() {
  Particle.publish("raha/player2status", player2status);
}

void winner(uint8_t wait) {
  Serial.print("void winner()");
  Serial.println();
  for (int j=0; j < 256; j = j + 15) {     // cycle less colors than before
     for (int q=0; q < 3; q++) {
       for (uint16_t i=0; i < strip.numPixels(); i=i+3) {
